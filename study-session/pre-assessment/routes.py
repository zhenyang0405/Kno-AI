"""
FastAPI routes for pre-assessment question generation and marking

Endpoints:
- POST /generate-questions: Generate 10 MCQ questions from a PDF material
- POST /mark-assessment: Mark a completed assessment with AI-powered feedback
"""
...
class RegisterMaterialRequest(BaseModel):
    """Request model for material registration"""
    user_id: int
...
class RegisterMaterialResponse(BaseModel):
    """Response model for material registration"""
    success: bool
    material_id: int
    message: str
...
def verify_questions_count(material_id: int, assessment_type: str) -> int:
    """
    Verify how many questions were saved for a material, optionally filtered by assessment_id

    Args:
        material_id: ID of the material
        assessment_id: Optional ID of the assessment to filter by
...
async def generate_questions(request: GenerateQuestionsRequest):
    """
    Generate 10 MCQ questions from a PDF material using ADK agent.

    This endpoint:
    1. Updates material status to 'processing'
    2. Invokes the question_generator agent
    3. Agent loads PDF from GCS and generates 10 questions
    4. Questions are saved to PostgreSQL database
    5. Updates material status to 'completed' or 'failed'

    Args:
        request: GenerateQuestionsRequest with material_id, storage_path, and storage_bucket
    ...
    """
    logger.info(f"=== GENERATE QUESTIONS START ===")
    ...
    try:
        # Check if a pre-assessment already exists for this material
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(
            """
            SELECT id, status, score, total_questions, summary
            FROM assessments WHERE material_id = %s AND user_id = %s AND assessment_type = 'pre'
        ...
        if existing_assessment:
            ...
            logger.info(f"Found existing assessment {assessment_id} with status '{assessment_status}' for material {request.material_id}")
            ...
            if assessment_status == AssessmentStatus.COMPLETED:
                ...
                logger.info(f"Assessment {assessment_id} is completed, returning results")
                return GenerateQuestionsResponse(
                    success=True,
                    material_id=request.material_id,
                    questions_generated=total_questions or 0,
                    message=f"Assessment already completed with score {score}/{total_questions}",
                    assessment_type="pre",
        ...
        # Check if questions already exist for this material (filtered by assessment_id if available)
        existing_count = verify_questions_count(request.material_id, "pre")
        if existing_count > 0:
            logger.info(f"Found {existing_count} existing questions for material {request.material_id}, skipping generation")
            return GenerateQuestionsResponse(
                success=True,
                material_id=request.material_id,
                questions_generated=existing_count,
                message=f"Questions already exist ({existing_count} found), skipping generation",
                assessment_type="pre",
        ...
        questions_count = verify_questions_count(request.material_id, "pre")
        logger.info(f"[3/4] Found {questions_count} questions in database")
        ...
        if questions_count == 10:
            ...
            logger.info(f"=== GENERATE QUESTIONS SUCCESS ===")

            return GenerateQuestionsResponse(
                success=True,
                material_id=request.material_id,
                questions_generated=questions_count,
                message=f"Successfully generated {questions_count} questions",
                assessment_type="pre"
            ...
            logger.error(f"=== GENERATE QUESTIONS FAILED (incomplete) ===")
        ...
        logger.error(f"=== GENERATE QUESTIONS ERROR ===")
...
async def start_assessment(request: StartAssessmentRequest):
    ...
    try:
        ...
        # Check if an assessment already exists for this material
        cursor.execute(
            """
            SELECT id FROM assessments
            WHERE user_id = %s AND material_id = %s AND assessment_type = 'pre'
            ORDER BY created_at DESC LIMIT 1
            """,
        ...
        cursor.execute(
            """
            INSERT INTO assessments (user_id, material_id, status, started_at, assessment_type)
            VALUES (%s, %s, %s, CURRENT_TIMESTAMP, 'pre') RETURNING id
            """,
...
async def get_questions(material_id: int):
    """Fetch all generated questions for a material"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        cursor.execute(
            ...
            FROM questions WHERE material_id = %s ORDER BY order_number
            """,
            (material_id,)
        )
...
async def mark_assessment(request: MarkAssessmentRequest):
    """
    Mark a completed assessment using ADK agent.

    This endpoint:
    1. Updates assessment status to 'processing'
    2. Invokes the assessment_marker agent
    3. Agent retrieves user answers and questions
    4. Agent calculates score and generates summary
    ...
    Returns:
        MarkAssessmentResponse with score and AI-generated summary
    ...
    try:
        ...
        user_message_text = f"assessment_id: {request.assessment_id}"
        ...
        return MarkAssessmentResponse(
            ...
            message=f"Successfully marked assessment {request.assessment_id}"
        )
...
async def health_check():
    ...
    return {"status": "healthy", "service": "pre-assessment-api"}
